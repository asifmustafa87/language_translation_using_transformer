...
class MultiHeadAttentionBlock(nn.Module):
    ...
    @staticmethod
    def attention(query, key, value, d_k, dropout: nn.Dropout, mask=None):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        x, self.attention_scores = MultiHeadAttentionBlock.attention(query, key, value, self.d_k, self.dropout)
...
if __name__ == "__main__":
    ...
    out_embeddings = input_embeddings(input_data)

    pos_encoding = PositionalEncoding(d_model, seq_len, dropout)
    pos_output = pos_encoding(out_embeddings)

    layer_norm = LayerNormalization(d_model)
    ln_output = layer_norm(pos_output)

    feed_forward_block = FeedForwardBlock(d_model, d_ff, dropout)
    ff_output = feed_forward_block(ln_output)

    multi_head_attention = MultiHeadAttentionBlock(d_model, n_heads, dropout)
    mha_output = multi_head_attention(ln_output, ln_output, ln_output)


    print("Input token shape:", input_data.shape)
    print("Embedded input shape:", out_embeddings.shape)
    print("Output with positional encoding shape:", pos_output.shape)
    print("Output with layer normalization shape:", ln_output.shape)
    print("Output with feedforward block shape:", ff_output.shape)
    print("Output with multihead Attention block shape:", mha_output.shape)
